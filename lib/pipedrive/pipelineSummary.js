...
async function pipelineSummary({ pipeline_id, getStageMap, sharedContext }) {
  const abiertos =
    sharedContext?.counts?.open ??
    (await countDealsByStatus("open", pipeline_id));

  const ganados =
    sharedContext?.counts?.won ??
    (await countDealsByStatus("won", pipeline_id));

  const perdidos =
    sharedContext?.counts?.lost ??
    (await countDealsByStatus("lost", pipeline_id));
